...
const {wait, getRandomDelay,typingTime} = require('../utils/delay.util');
    ...
    chatwootCtrl = async (req, res) => {
        ...
        try {
            ...
            if (body?.event === 'conversation_updated' && mapperAttributes.includes('assignee_id')) {
                ...
                if(idAssigned){
                    console.log(chalk.bgCyan(`[BLACKLIST] ADD THROUGH ASSIGNED AGENT `), chalk.green.bold(`${phone} agregado a la blacklist`));
                    bot.dynamicBlacklist.add(phone)
                }else{
                    console.log(chalk.bgCyan(`[BLACKLIST] REMOVE UNASSIGNED AGENT`), chalk.green.bold(`${phone} removido de la blacklist`));
            ...
            if(body?.event === 'message_created'){
                if (body?.content === '#off') {
                    console.log(chalk.bgCyan(`[BLACKLIST] ADD PHONE`) , chalk.green.bold(`numero ${body?.conversation?.meta?.sender?.phone_number.replace('+', '')} agregado a la blacklist`));
                    bot.dynamicBlacklist.add(body?.conversation?.meta?.sender?.phone_number.replace('+', ''));
                    return
                }
                if (body?.content === '#on') {
                    console.log(chalk.bgCyan(`[BLACKLIST] REMOVE PHONE`), chalk.green.bold(`numero ${body?.conversation?.meta?.sender?.phone_number.replace('+', '')} removido de la blacklist`));
